...
d3sparql.barchart = function(json, config) {
  ...
  var opts = {
    "label_x":  config.label_x_bar  || head[0],
    "label_y":  config.label_y_bar  || head[1],
    "var_x":    config.var_x_bar    || head[0],
    "var_y":    config.var_y_bar    || head[1],
    "width":    config.width    || 750,
    "height":   config.height   || 300,
    "margin":   config.margin   || 80 , // TODO: to make use of {top: 10, right: 10, bottom: 80, left: 80}
	"result":	config.result	|| "output",
  }
...
/*
  ...
  Options:
    config = {
	  ...
	  "output" : "output"	//id of generated svg
    }
...
d3sparql.scatterplot = function(json, config) {
  ...
  var opts = {
	...
	"output_bubble":	config.output_bubble	|| "output",
  }
  ...
  var circle = svg.selectAll("circle")
    ...
    .on("mouseout", function(){
		...
		for (var i=0, len=tmp.length; i<len; i++){
			if(d[tmp[4]].value == 'chugoku' || d[tmp[4]].value =='fukutoshin'){
				return "gold";
		        }
			else if(d[tmp[4]].value == 'kyusyu' || d[tmp[4]].value =='toshin'){
				return "red";
				}
			else if(d[tmp[4]].value == 'shikoku' || d[tmp[4]].value =='jotou'){
				return "darkorange";
				}
			else if(d[tmp[4]].value == 'kinki'	|| d[tmp[4]].value =='jonan'){
				return "violet";
				}
			else if(d[tmp[4]].value == 'chubu'	){
				return "springgreen";
				}
			else if(d[tmp[4]].value == 'kanto'	|| d[tmp[4]].value =='josai' ){
				return "greenyellow";
				}
			else if(d[tmp[4]].value == 'tohoku'	|| d[tmp[4]].value =='johoku'){
				return "Aqua";
				}
			else if(d[tmp[4]].value == 'hokkaido'){
...
d3sparql.triplemap = function(json, config) {
  var head = json.head.vars
  var data = json.results.bindings
  ...
  var opts = {
    ...
    "margin_x": config.margin_x || 80,
    "margin_y": config.margin_y || 40,
	"output_bubble":	config.output_bubble	|| "bubble",
	...
	"geojson":     config.geojson    || "japan.geojson",
    "mapname":      config.mapname     || "japan",
    "center_lat":   config.center_lat  || 34,
    "center_lng":   config.center_lng  || 137,
	"topowidth":    config.topowidth    || 850,
    "topoheight":   config.topoheight   || 300,
	"output_topomap":	config.output_topomap	|| "topomap",
	"label_x_bar":  config.label_x_bar  || head[0],
    "label_y_bar":  config.label_y_bar  || head[1],
    "var_x_bar":    config.var_x_bar    || head[0],
    "var_y_bar":    config.var_y_bar    || head[1],
    "margin":   config.margin   || 80 , // TODO: to make use of {top: 10, right: 10, bottom: 80, left: 80}
	"result":	config.result	|| "output",
  }
  ...
  var circle = svg1.selectAll("circle")
    ...
    .attr("cy", function(d) {return scale_y(d[opts.var_y].value)})
    .attr("r",  function(d) {return scale_r(d[opts.var_r].value)})
  var ax = svg1.append("g")
    .attr("class", "x axis")
    .attr("transform", "translate(0," + (opts.height - opts.margin_y) + ")")
    .call(axis_x)
  var ay = svg1.append("g")
    .attr("class", "y axis")
    .attr("transform", "translate(" + opts.margin_x + ",0)")
    .call(axis_y)
  ax.append("text")
  ...
  d3.json(opts.geojson, function (geoData) {
	overlay.destroy();
	...
	//*****bar chart part*****//
  var scale_x_bar = d3.scale.ordinal().rangeRoundBands([0, opts.width - opts.margin], 0.1)
  var scale_y_bar = d3.scale.linear().range([opts.height - opts.margin, 0])
  var axis_x_bar = d3.svg.axis().scale(scale_x_bar).orient("bottom")
  var axis_y_bar = d3.svg.axis().scale(scale_y_bar).orient("left")  // .ticks(10, "%")
  scale_x_bar.domain(data.map(function(d) {return d[opts.var_x_bar].value}))
  scale_y_bar.domain(d3.extent(data, function(d) {return parseInt(d[opts.var_y_bar].value)}))
...
//cleanup old graph 
  ...
  var ax = svg2.append("g")
    .attr("class", "axis x_bar")
    .attr("transform", "translate(" + opts.margin + "," + (opts.height - opts.margin) + ")")
    .call(axis_x_bar)
  var ay = svg2.append("g")
    .attr("class", "axis y_bar")
    .attr("transform", "translate(" + opts.margin + ",0)")
    .call(axis_y_bar)
  var bar = svg2.selectAll(".bar")
    ...
    .attr("transform", "translate(" + opts.margin + "," + 0 + ")")
    .attr("class", "bar")
    .attr("x", function(d) {return scale_x_bar(d[opts.var_x_bar].value)})
    .attr("width", scale_x_bar.rangeBand())
    .attr("y", function(d) {return scale_y_bar(d[opts.var_y_bar].value)})
    .attr("height", function(d) {return opts.height - scale_y_bar(parseInt(d[opts.var_y_bar].value)) - opts.margin})
	.on("mouseover", function(){
        d3.select(this) 
            .attr("style", "fill:rgb(0,0,255)")
			.append("title")
  ...
  ax.append("text")
    .attr("class", "label")
    .text(opts.label_x_bar)
    .style("text-anchor", "middle")
    .attr("transform", "translate(" + ((opts.width - opts.margin) / 2) + "," + (opts.margin - 5) + ")")
  ay.append("text")
    .attr("class", "label")
    .text(opts.label_y_bar)
    .style("text-anchor", "middle")
    .attr("transform", "rotate(-90)")
    .attr("x", 0 - (opts.height / 2))
    .attr("y", 0 - (opts.margin - 20))
...
d3sparql.geomap = function(json, config){
  ...
  console.log(data)
	...
	"topowidth":    config.topowidth    || 850,
    "topoheight":   config.topoheight   || 300,
	"output_geomap":	config.output_topomap	|| "geomap",
	"basemap":	config.basemap	|| null,
 ...
 console.log(size)
  ...
  d3.json(opts.topojson, function(topojson_map) {
    ...
    svg.selectAll(".place-label")
      ...
      .attr("dx", "-1.5em")
      .text(function(d) { return d.properties.name; })  //default is name_local instead of name
  })
}


//tmp try
d3sparql.modifiedmap = function(json, config) {
  var head = json.head.vars
  var data = json.results.bindings
  ...
  var svg = d3.select("body")
	...
	geo.forEach(function(d, i) {
		...
		centroid.feature = d;
		var tmp = color(size[d.properties.name])
		global_color.push(tmp);
		global_lat.push(d.properties.latitude);
		global_lon.push(d.properties.longitude);
	...
	var triangles = d3.geom.voronoi().triangles(nodes);
		...
		//力学グラフ生成
		var force = d3.layout.force().size([opts.width, opts.height]);		
		force.gravity(0.01)
			.charge(0.01)
			.nodes(nodes)
			.links(links)
		...
		svg.selectAll(".place-label")
			...
			.attr("transform", function(d,i) {
				return "translate(" + projection([global_lon[i], global_lat[i]]) + ")"
				})
			.call(force.drag) //forceレイアウトのドラッグビヘイビアを設定
			.attr("dx", "-1.5em")
			.text(function(d) { return d.properties.name; })  //default is name_local instead of name
...
d3sparql.cartgram = function(json, config){
  var head = json.head.vars
  var data = json.results.bindings
  ...
  d3.json(opts.topojson, function(topojson_map) {
    ...
    svg.selectAll("path")
      ...
      .attr("stroke", "black")
      .style("fill", function(d, i) {
		total += Math.sqrt(size[d.properties.name]);
		count += 1;
		//console.log(d.rpoperties.name)
        // map SPARQL results to colors
        return color(size[d.properties.name])    //default is name_local instead of name
      })
	  .attr("transform", function(d) {
	   ...
	   .style("stroke-width", function(d) {
			return 1 / Math.sqrt(size[d.properties.name] /200 || 1);
		})
    ...
    svg.selectAll(".place-label")
      ...
      .attr("transform", function(d) {
        ...
        var lng = d.properties.longitude
        return "translate(" + projection([lng, lat]) + ")"
		//+ "scale(" + Math.sqrt(size[d.properties.name]) + ")"
...
d3sparql.namedmap_mod = function(json, config){
  var head = json.head.vars
  var data = json.results.bindings
  ...
  d3.json(opts.topojson, function(topojson_map) {
    ...
    svg.selectAll(".place-label")
	  ...
	  .attr("transform", function(d) {
        ...
        var lng = d.properties.longitude
        return "translate(" + projection([lng, lat]) + ")"
		+ "scale(" + Math.sqrt(size[d.properties.name]|| 0) + ")"
      })
      .attr("dx", "-1.5em")
      .text(function(d) { return d.properties.name; })  //default is name_local instead of name
